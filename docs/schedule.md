...
| 이 변경 PR 머지 + 샘플 PR에서 `Validate` 초록 확인 | 커밋/PR → Actions `Validate` 통과 확인. 의도적 breakage 한 번(선택). 통과 시 `docs/resolved` 이동 | [PR CI 검증 파이프라인](issues/2026-09-11-pr-ci-validation-pipeline.md) | 적용 |
